	...
	class pool : public app
	{
		...
		void world_rot()
		{
			...
			if (is_key_down(key_lmb))
			{
				...
				app_scene->add_shape(modelToWorld, new mesh_sphere(vec3(0, 0, 0), 10.0f), new material(vec4(1, 0, 0, 1)), true);
			}
		}

	public:
		pool(int argc, char**argv) :app(argc, argv)
		{
		...
		void app_init()
		{
			...
			app_scene->get_camera_instance(0)->get_node()->translate(vec3(0, 100, 1000));
			...
			if (tablemeshes.size())
			{
				...
				{
					app_scene->add_shape(modelToWorld, tablemeshes[0]->get_mesh(), poolMat, false);
